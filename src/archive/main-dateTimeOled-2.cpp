...
void setup()
{
    ...
    u8g2.setPowerSave(1); // put OLED into power save mode (display off, but u8g2 internal state is still awake so we can just call sendBuffer() to wake it up again without a full re-init) since we start with the OLED powered down and we don't want to try to talk to it until the button is pressed.
}
...
void loop()
{
    ...
    if (digitalRead(TEST_PB_PIN) == LOW)
    {
        ...
        {
            Serial.println("OLED found powered down. Powering up.");
    ...
    else
    {
        if (!OLED_Powered_Down)
        {
            Serial.println("OLED powering down. Button released");
            digitalWrite(TEST_LED_PIN, LOW);
            u8g2.setPowerSave(1); // put OLED into power save mode (display off)
            OLED_Powered_Down = true;
        }
        else
        {
            Serial.println("OLED already powered down. Button released.");
        }
    }
